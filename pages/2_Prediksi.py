...
try:
    from utils.sarima_engine import (
        forecast as sarima_forecast,
        model_exists as sarima_exists,
        load_forecast_from_db as sarima_load_fc_db,
    )
    SARIMA_OK = True
...
VILLA_COLORS = (
    {r["villa_name"]: r["color_hex"] for _, r in df_villas.iterrows()}
...
for vn in villa_names:
    avg_tmp = df_occ[df_occ["villa_name"] == vn]["occupancy_pct"].mean() or 0
    if avg_tmp < 50:
        risk_villas_count += 1
    if avg_tmp > best_occ:
        best_occ   = avg_tmp
...
occ_2026_actual = (
    df_occ[df_occ["year"] == 2026]["occupancy_pct"].mean()
    if (df_occ["year"] == 2026).any()
    else None
)
...
with e3:
    rc_cls = "negative" if risk_villas_count > 3 else "positive" if risk_villas_count == 0 else ""
    st.markdown(f"""<div class='exec-metric'>
      <div class='exec-metric-label'>Vila Rata-rata &lt;50%</div>
      <div class='exec-metric-value'>{risk_villas_count}/{len(villa_names)}</div>
      <div class='exec-metric-delta {rc_cls}'>ocupansi rata-rata &lt;50%</div>
    </div>""", unsafe_allow_html=True)

with e4:
    st.markdown(f"""<div class='exec-metric'>
      <div class='exec-metric-label'>Vila Terbaik</div>
      <div class='exec-metric-value' style='font-size:18px'>{best_villa.replace(" Villas","")}</div>
...
# Check model trained status safely
model_trained = False
try:
...
try:
    ...
    with t1:
        if not SARIMA_OK:
            bcls, btxt = "err",  "Import Error"
        elif model_trained:
            bcls, btxt = "ok",   "Model Tersedia"
        else:
        ...
        if not train_disabled:
            if st.button("🚀 Train Model", use_container_width=True, key="btn_train"):
                with st.spinner(f"Melatih {sel_villa}... (3-8 menit)"):
                    try:
                        series = (
                            df_occ[df_occ["villa_name"] == sel_villa]
                            .copy()
                            .set_index("date")["occupancy_pct"]
                            .sort_index()
                        )
    ...
    with t3:
        retrain_enabled = TRAIN_OK and sarima_train is not None and model_trained
        if retrain_enabled:
    ...
    with t4:
        if not SARIMA_OK:
            st.markdown(
                f"<div style='font-size:11px;color:#C0392B;background:#FFF0F0;border:1px solid #FFC5C5;"
                f"border-radius:8px;padding:8px 12px;font-family:DM Mono,monospace'>"
                f"❌ Import error: {SARIMA_ERR[:120]}</div>",
                unsafe_allow_html=True,
            )
        elif not model_trained:
            st.markdown(
                "<div style='font-size:11px;color:#A05C00;background:#FFF8EC;border:1px solid #FFD88A;"
                "border-radius:8px;padding:8px 12px'>⚠️ Model belum dilatih. Klik <b>Train Model</b> untuk memulai.</div>",
                unsafe_allow_html=True,
            )
        elif sel_db_row:
            ...
            st.markdown(
                f"<div style='font-size:11px;color:#1A7C44;background:#EDFAF2;border:1px solid #B2EAC8;"
                f"border-radius:8px;padding:8px 12px;font-family:DM Mono,monospace'>"
                f"✅ Order: {order_str} &nbsp;|&nbsp; S={sel_db_row.get('m_used','?')} &nbsp;|&nbsp; "
                f"Train: {sel_db_row.get('n_train','?')} minggu &nbsp;|&nbsp; "
                f"MAPE: {sel_db_row.get('mape','?')}% &nbsp;|&nbsp; AIC: {sel_db_row.get('aic','?')} &nbsp;|&nbsp; "
                f"<span style='color:#94a3b8'>Trained: {trained_at}</span></div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                "<div style='font-size:11px;color:#1A7C44;background:#EDFAF2;border:1px solid #B2EAC8;"
                "border-radius:8px;padding:8px 12px'>✅ Model tersedia. Detail tidak ditemukan di DB.</div>",
                unsafe_allow_html=True,
            )
...
def _aggregate_weekly_fc(raw_fc: pd.DataFrame) -> pd.DataFrame:
    """Aggregate a weekly forecast dataframe to monthly."""
    ...
    out["date"] = pd.to_datetime([f"2026-{m:02d}-01" for m in out["month_num"]])
        ...
        rows_naive = [
            {
                ...
                "predicted": round(float(np.clip(avg_by_m.get(d.month, overall), 0, 100)), 1),
                "upper":     round(min(100, float(np.clip(avg_by_m.get(d.month, overall), 0, 100)) * 1.10), 1),
                "lower":     round(max(0,   float(np.clip(avg_by_m.get(d.month, overall), 0, 100)) * 0.90), 1),
            }
            for d in pd.date_range(FC_START, end="2026-06-30", freq="W")
        ]
        fc_naive = pd.DataFrame(rows_naive)
        fc_df = fc_naive.groupby(["month", "month_num"]).agg(
            {"predicted": "mean", "lower": "min", "upper": "max"}
        ).reset_index()
...
try:
    if has_actual:
        st.markdown(
            f"<div class='section-label'>Prediksi vs Aktual Okupansi 2026</div>"
            f"<div style='background:#EEF3FF;border:1px solid #C5D7FF;border-radius:10px;"
            f"padding:10px 16px;margin-bottom:14px;font-size:12px;color:#1d4ed8'>"
            f"✅ <b>Data aktual tersedia</b> — {len(actual_monthly)} bulan &nbsp;·&nbsp; "
            f"Garis oranye = realisasi &nbsp;·&nbsp; Garis biru = prediksi {model_label}{cache_label}</div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f"<div class='section-label'>Prediksi Okupansi 2026</div>"
            f"<div style='background:#FFF8EC;border:1px solid #FFD88A;border-radius:10px;"
            f"padding:10px 16px;margin-bottom:14px;font-size:12px;color:#A05C00'>"
            f"⏳ <b>Belum ada data aktual 2026</b> &nbsp;·&nbsp; Model: {model_label}{cache_label}</div>",
            unsafe_allow_html=True,
        )
...
try:
    ...
    st.markdown(
        f"<div style='background:#F5F7FF;border:1px solid #D8E0FF;border-radius:10px;"
        f"padding:12px 16px;margin-bottom:16px;font-size:12px;color:#3D5BC0'>"
        f"<b>📐 Model:</b> {model_label} &nbsp;|&nbsp; <b>Vila:</b> {sel_villa} &nbsp;|&nbsp; "
        f"<b>S = {m_info}</b> &nbsp;|&nbsp; <b>Horizon:</b> {fc_horizon_months} bulan &nbsp;|&nbsp; "
        f"{'💾 Forecast dari DB cache' if forecast_from_cache else '🔄 Forecast dihitung baru'}</div>",
        unsafe_allow_html=True,
    )
    ...
    if not fc_df.empty:
        ...
        for _, row in tbl_df.iterrows():
            ...
            bar_html = (
                f"<div style='background:#F5F5F3;border-radius:100px;height:8px;overflow:hidden'>"
                f"<div style='background:{oc};width:{bar_w}%;height:100%;border-radius:100px'></div></div>"
            )

            rows_html += (
                f"<tr>"
                f"<td style='width:22%'><b style='font-size:15px'>{month_label}</b></td>"
                f"<td style='width:48%'><div style='display:flex;align-items:center;gap:14px'>"
                f"<span style='font-family:DM Mono,monospace;font-size:22px;font-weight:700;color:{oc};min-width:72px'>{occ_pred:.1f}%</span>"
                f"<div style='flex:1'>{bar_html}</div></div></td>"
                f"<td style='width:30%'>{actual_cell}</td>"
                f"</tr>"
            )
            ...
            cards_html += (
                ...
                f"<b style='font-size:16px'>{month_label}</b>"
                f"<div style='text-align:right'>"
                f"<span style='font-family:DM Mono,monospace;font-size:22px;font-weight:700;color:{oc}'>{occ_pred:.1f}%</span>"
                f"{'<br>' + actual_cell if has_row_actual else ''}</div></div>"
                f"{bar_html}</div>"
            )

        actual_note = "ter-upload" if has_actual else "belum ada data"
        st.markdown(
            f"<div class='forecast-table-wrapper'>"
            f"<table class='forecast-table'><thead><tr>"
            f"<th style='width:22%'>Bulan</th>"
            f"<th style='width:48%'>Prediksi Okupansi <span style='font-weight:400;text-transform:none;letter-spacing:0;font-size:9px'>{model_label}</span></th>"
            f"<th style='width:30%'>Aktual 2026 <span style='font-weight:400;text-transform:none;letter-spacing:0;font-size:9px'>{actual_note}</span></th>"
            f"</tr></thead><tbody>{rows_html}</tbody></table></div>"
            f"<div class='forecast-cards-wrapper'>{cards_html}</div>",
            unsafe_allow_html=True,
        )
...
if has_actual and not fc_df.empty:
    try:
        ...
        metrics = [
            (ka, "MAPE Aktual", f"{mape_live:.1f}%",  "< 10% Sangat Baik · < 20% Baik",      "#1A7C44" if mape_live < 10  else "#A05C00" if mape_live < 20  else "#C0392B"),
            (kb, "MAE",         f"{mae_live:.1f}pp",  "Mean Absolute Error",                   "#1A7C44" if mae_live  < 5   else "#A05C00" if mae_live  < 15  else "#C0392B"),
            (kc, "RMSE",        f"{rmse_live:.1f}pp", "Root Mean Squared Error",               "#1A7C44" if rmse_live < 5   else "#A05C00" if rmse_live < 15  else "#C0392B"),
            (kd, "Bias",        f"{bias:+.1f}pp",     "+ = under-forecast · − = over-forecast","#1d4ed8" if abs(bias) < 3   else "#A05C00"),
        ]
        for col, label, val, note, color in metrics:
            with col:
                st.markdown(
                    f"<div class='exec-metric'><div class='exec-metric-label'>{label}</div>"
                    f"<div class='exec-metric-value' style='color:{color};font-size:26px'>{val}</div>"
                    f"<div class='exec-metric-delta'>{note}</div></div>",
                    unsafe_allow_html=True,
                )
        ...
        fig_err.add_trace(go.Bar(
            ...
            hovertemplate=(
                ...
                "Prediksi: %{customdata[1]:.1f}%<br>Error: %{y:+.1f}pp<br>"
                "APE: %{customdata[2]:.1f}%<extra></extra>"
            ),
        ))
        fig_err.add_hline(y=0, line_color="#334155", line_width=1.5)
        for _, row in eval_df.iterrows():
            y_ann = row["error"] + (2.5 if row["error"] >= 0 else -4.5)
        ...
        fig_err.update_layout(
            **LAYOUT,
            ...
            title=dict(
                ...
                font=dict(size=14, color="#111", family="DM Serif Display"),
                x=0,
            ),
            xaxis=dict(
                showgrid=False, tickangle=-35, tickfont_size=11, linecolor="#E8E8E5",
        ...
        with st.expander("📋 Detail tabel evaluasi", expanded=False):
            ...
            def style_error(v):
                if not isinstance(v, (int, float)):
                    return ""
                return "color: #1A7C44; font-weight: 600" if v > 0 else "color: #C0392B; font-weight: 600" if v < 0 else ""
...
try:
    trained_villas = []
    if SARIMA_OK and callable(sarima_exists or None):
        for vn in villa_names:
            try:
                if sarima_exists(vn):
    ...
    if trained_villas:
        ...
        def format_sarima_order(db_row: dict) -> str:
            ao = db_row.get("arima_order", "")
            so = db_row.get("seasonal_order", "")
            m  = str(db_row.get("m_used", 52))
            m_sub = SUB_MAP.get(m, f"[{m}]")
            so_disp = re.sub(r",(\d+)\)$", f",{m_sub})", str(so)) if so else ""
            return f"{ao}{so_disp}" if ao else "—"

        def order_html(order_str: str) -> str:
            if ")(" in order_str:
                p1, p2 = order_str.split(")(")
                return (
                    f"<span style='font-size:13px;font-weight:700;font-family:DM Mono,monospace;color:#7C3AED'>{p1})</span>"
                    f"<span style='font-size:12px;font-weight:600;font-family:DM Mono,monospace;color:#A855F7'>({p2}</span>"
                )
            return f"<span style='font-size:13px;font-weight:700;font-family:DM Mono,monospace;color:#7C3AED'>{order_str}</span>"
        ...
        for vn in villa_names:
            ...
            if not is_trained or not db:
                rows_acc += (
                    f"<tr style='opacity:0.40'>"
                    f"<td>{dot}<b>{vn}</b><br>"
                    f"<span style='font-size:10px;color:#aaa;font-family:DM Mono,monospace'>{area}</span></td>"
                    f"<td colspan='4' style='font-size:11px;color:#bbb;font-family:DM Mono,monospace;text-align:center'>⏳ Belum dilatih</td>"
                    f"</tr>"
                )
                continue

            mape_v = db.get("mape"); rmse_v = db.get("rmse"); aic_v = db.get("aic")
            m_val  = db.get("m_used", 52)
            n_tr   = db.get("n_train", "—"); n_cy = db.get("n_cycles", "—")
            tr_at  = str(db.get("trained_at", ""))[:10]
            aic_disp   = fmt_v(round(float(aic_v)) if aic_v else None, "", 0)
            o_str      = format_sarima_order(db)

            rows_acc += (
                f"<tr>"
                f"<td>{dot}<b>{vn}</b>"
                f"<br><span style='font-size:10px;color:#aaa;font-family:DM Mono,monospace'>{area}</span>"
                f"<br><span style='font-size:9px;color:#cbd5e1;font-family:DM Mono,monospace'>trained {tr_at}</span></td>"
                f"<td><span style='font-size:17px;font-weight:700;font-family:DM Mono,monospace;color:{mape_color(mape_v)}'>{fmt_v(mape_v,'%',2)}</span></td>"
                f"<td><span style='font-size:15px;font-weight:700;font-family:DM Mono,monospace;color:{rmse_color(rmse_v)}'>{fmt_v(rmse_v,'',2)}</span></td>"
                f"<td><span style='font-size:13px;font-family:DM Mono,monospace;color:#3D6BE8'>{aic_disp}</span></td>"
                f"<td>{order_html(o_str)}"
                f"<br><span style='font-size:9px;color:#94a3b8;font-family:DM Mono,monospace'>S={m_val} · {n_tr} minggu · {n_cy} siklus</span></td>"
                f"</tr>"
            )

        st.markdown(
            f"<table class='summary-acc-table'><thead><tr>"
            f"<th>Vila</th>"
            f"<th>MAPE<br><span style='font-weight:400;font-size:8px;text-transform:none;letter-spacing:0'>Error rata-rata %</span></th>"
            f"<th>RMSE<br><span style='font-weight:400;font-size:8px;text-transform:none;letter-spacing:0'>Error kuadrat</span></th>"
            f"<th>AIC<br><span style='font-weight:400;font-size:8px;text-transform:none;letter-spacing:0'>Fit model</span></th>"
            f"<th>Model SARIMA<br><span style='font-weight:400;font-size:8px;text-transform:none;letter-spacing:0'>(p,d,q)(P,D,Q)[S]</span></th>"
            f"</tr></thead><tbody>{rows_acc}</tbody></table>",
            unsafe_allow_html=True,
        )
...
def build_monthly_data(villa_name: str, year_filter=None):
    ...
    if year_filter:
        occ = occ[occ["year"].isin(year_filter)]
    occ["period"] = occ["date"].dt.to_period("M").astype(str)

    monthly_occ = (
        occ.groupby("period")["occupancy_pct"]
        .mean()
        .reset_index()
    ...
    corr_info = {
        "r": 0, "live": False,
        "elastisitas": vi.get("elastisitas", "Semi-Elastis"),
        "area": vi.get("area", ""),
    }
    ...
    if year_filter:
        fin = fin[fin["year"].isin(year_filter)]
    fin_clean = (
        fin[fin["for_modeling"] == 1].copy()
        if "for_modeling" in fin.columns
        else fin[fin["avg_daily_revenue"] > 0].copy()
    )
    ...
    monthly_adr = (
        fin_clean.groupby("period")["avg_daily_revenue"]
        .mean()
        .reset_index()
    ...
    monthly = (
        pd.merge(monthly_occ, monthly_adr, on="period", how="left")
        .sort_values("period")
        .reset_index(drop=True)
    )
    daily = pd.merge(
        occ[["date", "occupancy_pct"]],
        fin_clean[["date", "avg_daily_revenue"]],
        on="date", how="inner",
    ).dropna()
...
try:
    all_areas = sorted({VILLA_INSIGHTS.get(v, {}).get("area", "") for v in villa_names} - {""})
    ...
    display_villas = sel_villas_desc or fv
    year_filter    = sel_years_desc  or None

    if not display_villas:
        st.info("Pilih minimal satu vila.")
    else:
        tabs = st.tabs([v.replace(" Villas", "") for v in display_villas])
        for tab, vn in zip(tabs, display_villas):
            with tab:
                try:
                    vc4 = get_color(vn)
                    ...
                    if "adr_m" not in monthly.columns:
                        monthly["adr_m"] = np.nan

                    # ── Dual-axis chart ──
                    fig_dual = make_subplots(specs=[[{"secondary_y": True}]])
                    fig_dual.add_trace(go.Bar(
                        x=monthly["period"], y=monthly["occ"],
                        name="Okupansi (%)",
                        marker_color=f"rgba({r_hex},{g_hex},{b_hex},0.55)",
                        marker_line_color=f"rgba({r_hex},{g_hex},{b_hex},0.85)",
                    ...
                    if not adr_data.empty:
                        fig_dual.add_trace(go.Scatter(
                            x=adr_data["period"], y=adr_data["adr_m"],
                            name="ADR (Juta Rupiah)",
                            mode="lines+markers",
                    ...
                    fig_dual.update_layout(
                        **LAYOUT,
                        ...
                        margin=dict(l=0, r=0, t=60, b=40),
                        title=dict(
                            text=f"Tren Musiman — {vn} &nbsp;<span style='font-size:12px;color:#666'>r = {r_val:+.2f} · n={n_obs} · {src}</span>",
                    ...
                    if not daily_merged.empty and len(daily_merged) >= 5:
                        ...
                        pt_alpha = [
                            f"rgba({r_hex},{g_hex},{b_hex},0.65)" if y >= adr_med
                            else f"rgba({r_hex},{g_hex},{b_hex},0.30)"
                            for y in sy
                        ]
                        ...
                        fig_sc.add_trace(go.Scatter(
                            ...
                            marker=dict(size=6, color=pt_alpha, line=dict(color=vc4, width=0.5)),
                            name="Hari Observasi",
                        ...
                        fig_sc.add_annotation(
                            ...
                            showarrow=False,
                            bgcolor="#fff", bordercolor=vc4, borderwidth=1,
                        ...
                        # ── Correlation & price profile cards ──
                        abs_r = abs(r_sc)
                        ...
                        adr_med_d   = cd2.get("adr_median", 0)
                        adr_hi_d    = cd2.get("adr_max", 0)
                        adr_lo_d    = cd2.get("adr_min", 0)
